---
title: "MA615 Final Porject---NYC Restaurant Week Analysis"
author: "Yunyi Zhang"
date: "16/12/2019"
output: pdf_document
---
...
1. Summary and analysis on different type of restaurant.
2. Relationship between type and rating
3. Word Analysis

## 2. Read dataset and clean the data
...
## 3. Breif summary of the dataset
As shown, there are in total 17 type of restaurant(by food), and the average rating ranged from 3.75 to 4.88. Number of reviews varies from and up to 18139 reviews.
```{r, echo = TRUE}
length(unique(datac$restaurant_main_type))
length(unique(datac$restaurant_type))
range(datac$average_review)
range(datac$review_count)
```

## 4. Tables and Graphs
# 4.1 Number of resaurant for each type
```{r, echo=FALSE}
...
\newpage
# 4.2 Average rating for each type
...
\newpage
# 4.3 Average rating versus Number of Reviews
...
## 5.1 Word cloud for restaurant rating 3.8~4.2
...
\newpage
## 5.2 The wordCloud for rating 4.2~4.6
...
head(dm,8)
wordcloud(words=dm$word, freq=dm$freq, max.words = 100, random.order = FALSE)#wordcloud(second_tier, max.words = 100, random.order = FALSE)
```

\newpage
# 5.3 The wordCloud for rating 4.6~5.0
...
head(dm2,8)
wordcloud(words=dm2$word, freq=dm2$freq, max.words = 100, random.order = FALSE)#wordcloud(second_tier, max.words = 100, random.order = FALSE)
```
